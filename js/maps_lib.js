...
var MapsLib = {
  ...
  //NOTE: numeric IDs will be depricated soon
   fusionTableId:      "1wrTWE0Wi0GoZ1BJHE6bBkOfUEpwYNMrK3Jvk1O_V",
  ...
  doSearch: function(location) {
    ...
    MapsLib.poly = new google.maps.FusionTablesLayer({
          query: {
            select: 'effet',
            from: '1VoeYTNl2AkpV_zTuRezRa3bzU-NXkCnmMRYrbSpl',
              }, 
          styles: [{
...
MapsLib.layer = new google.maps.FusionTablesLayer({
          ...
          styles: [{
            ...
            where: 'MJA>=1000 AND MJA<2000 ',
            polylineOptions: {
              strokeColor: "#CCFF00"
            }
          },{
            where: 'MJA>=2000 AND MJA<5000',
            polylineOptions: {
              strokeColor: "#FF6699"
            }},
      {
            where: 'MJA>=5000 AND MJA<10000',
            polylineOptions: {
              strokeColor: "#0033CC"
            }},
      {
            where: 'MJA>=10000 AND MJA<15000',
            polylineOptions: {
              strokeColor: "#FF0000"
            }},
      {
  ...
  var type_column_lum = "'lumi'";
  if ( $("#lum-tout").is(':checked')) whereClause += " AND " + type_column_lum + " IN('0','1','2','3')";
  if ( $("#lum1").is(':checked')) whereClause += " AND " + type_column_lum + "=3";
  if ( $("#lum2").is(':checked')) whereClause += " AND " + type_column_lum + "=2";
  if ( $("#lum3").is(':checked')) whereClause += " AND " + type_column_lum + "=0";
  if ( $("#lum4").is(':checked')) whereClause += " AND " + type_column_lum + "=1";
...
/*
 ...
 // if ( $("#grv1").is(':checked')) whereClause += " AND " + type_column_grv + "=0";
  ...
  if ( $("#an5").is(':checked')) {whereClause += " AND " + type_column_an + "=2009"; MapsLib.poly.setMap(map);}
  if ( $("#an6").is(':checked')) {whereClause += " AND " + type_column_an + "=2010" ; MapsLib.layer.setMap(map); }
